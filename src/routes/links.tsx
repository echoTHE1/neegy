...
<<<<<<< HEAD
    try {
      ...
      if (!result.ok) {
        ...
        setPosts([]);
        setLoadError(
          "NEEGY Links could not reach the server. Check the Supabase server configuration and try again.",
        );
        return;
      }
      ...
      console.error("[NEEGY Links] load failed:", error);
      setPosts([]);
      setLoadError(
...
<<<<<<< HEAD
      try {
        ...
        console.error("[NEEGY Links] owner session check failed:", error);
      }
...
<<<<<<< HEAD
    try {
      ...
      console.error("[NEEGY Links] owner unlock failed:", error);
      setUnlockError(
        "Owner access is unavailable. Check the deployment's Supabase and owner-secret configuration.",
      );
    } finally {
      setUnlocking(false);
    }
...
<<<<<<< HEAD
            ...
            <GlassCard className="p-8 text-center">
              ...
              <Button className="mt-5" onClick={() => void load(localStorage.getItem(OWNER_TOKEN_KEY))}>
                Retry
              </Button>
            </GlassCard>
=======
